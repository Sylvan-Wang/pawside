-- Pawside Minimum P1 §2.3: Program Day prescriptions must be queryable and
-- generatable idempotently.
--
-- The released public.create_session_prescription_for_cycle(uuid) can only
-- materialise the prescription for the enrollment's CURRENT next_split_key. That
-- makes an out-of-order Program Day (PRD §2.3 / acceptance Case E: Day 3 before
-- Day 2) impossible, because Day 3's prescription does not exist yet.
--
-- This migration adds an explicit, idempotent generator for one named Program Day
-- of one cycle. It:
--   * never changes enrollment.next_split_key,
--   * never changes cycle status,
--   * never touches an existing non-cancelled prescription for that split,
-- so generating/viewing a Program Day is NOT a progression side effect.
--
-- The released create_session_prescription_for_cycle(uuid) is left untouched.
...
begin
  ...
  if p_split_key not in ('push', 'pull', 'legs') then
...
comment on function public.create_program_day_prescription(uuid, text) is
  'Minimum P1: idempotently materialises one named Program Day of one cycle. Never advances next_split_key, cycle status, or Program Day completion.';
